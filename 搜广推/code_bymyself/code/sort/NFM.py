...
class BiInteractionPooling(nn.Module):
    ...
    def forward(self,input):
        ...
        square_of_sum = torch.square(torch.sum(input,dim=1,keepdim=False))
        sum_of_square = torch.sum(input * input, dim=1, keepdim=False)
...
class DeepFM(nn.Module):
    def __init__(self, Linearfeature, DNNFeature, dense_dim,
        ...
        # 双线性池化部分,唯一不同的部分
        self.Pooling = BiInteractionPooling()
    ...
    def forward(self, linear_dense_data, linear_sparse_data, dnn_dense_data, dnn_sparse_data):
        ...
        pool_input = torch.stack([emb(dnn_sparse_data[:, i]) for i, emb in enumerate(self.dnn_embeddings)], dim=1)
        pool_embed = self.Pooling(pool_input)

        # DNN层
        dnn_input = torch.cat([dnn_dense_data, pool_embed], dim=1)
