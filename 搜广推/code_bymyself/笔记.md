...
+ 原理
  + 输入特征为一个用户id和项目id，先将其转换为onehot向量，然后映射成一个指定维度的稠密向量。能够很好的调整解决==冷启动问题==。
  + GMF（广义矩阵分解部分）：将用户与物品的特征矩阵对应做逐元素点积，通过线性方法学习特征
  + MLP：先分别设置用户MLP与物品MLP，在经过特征组合后拼接embedding，经过一个线性层得到MLP的输出
  + 最终：拼接GMF与MLP的特征，经过线性层得到最后的输出。
+ 回归方法：均方误差
$$
L_{sqr}=\sum_{(u,i)\in y\cup y^-}w_{ui}(y_{ui}-\hat{y}_{ui})^2
$$ 其中$w_{ui}$为一个超参数，给每个样本赋权重。
+ 似然函数
$$
p(y,y^-|P,Q,\Theta_f)=\prod_{(u,i)\in{y}}\hat{y}_{ui}\prod_{(u,j)\in{y^-}}(1-\hat{y}_{uj})
$$
+ 目标函数->对似然函数取负对数（1. 表示$Loss$函数; 2. 消除小数乘法下溢出问题）
$$
...
$$ 使用随机梯度下降$（SGD）$来进行训练优化。这个函数和交叉熵损失函数$（binary cross-entropy loss，又被称为log loss）$是一样的。通过在NCF上使用这样一个概率处理$（probabilistic- treatment）$，把隐性反馈的推荐问题当做一个二分类问题来解决。对于负样本 $y^-$ ，每次迭代**均匀地从未观察到交互作用中采样**（作为负样本）并且对照可观察到交互的数量，控制采样比率。(正负样本的比例控制在召回系统中非常重要)
**MF是NCF模型的一个特例**

## DeepCrossing
==第一个完整解决了特征工程、稀疏向量稠密化，多层神经网络进行优化目标拟合等一系列深度学习在推荐系统的应用问题==，应用场景是点击率预测，给定的数据集中分为离散特征和连续特征，对离散特征就是先onehot再稠密化，连续特征将其通过log变换成一个数（只是一种变换方式）。
+ 模型结果与原理
  + 需解决问题
    1. 离散类特征编码后过于稀疏，不利于直接输入神经网络训练，需要解决**稀疏特征向量稠密化**的问题
    2. 如何解决特征**自动交叉组合**的问题
  ...
  + 各层作用
    1. Embedding Layer
    只针对离散型变量，首先转换为onehot变量，然后利用已有的embedding函数将其转换为一个指定维度的稠密向量（一般维度不会太大）
    2. Stacking Layer
    先将数值型特征拼接起来，再将embedding拼接起来，最后将两部分拼接作为模型的输入。
    3. MLP + Residual Connection
    先利用两层MLP对特征进行非线性变换，然后与原始输入特征做残差链接，最后经过一层线性层+激活函数得到最后预测。

## PNN
## Wide and Deep

模型既可以从历史数据中学习到高频共现的特征组合能力（记忆能力），也可以利用特征之间的传递性去探索历史数据中从未出现过的特征（泛化性）。
...
- wide部分是一个广义的线性模型，输入的特征主要有两部分组成，一部分是原始的部分特征，另一部分是原始特征的交叉特征(cross-product transformation)，对于交互特征可以定义为：
  ...
  对于wide部分训练时候使用的优化器是带$L_1$正则的FTRL算法(Follow-the-regularized-leader)，而L1 FTLR是非常注重模型稀疏性质的，也就是说W&D模型采用L1 FTRL是想**让Wide部分变得更加的稀疏**，即Wide部分的大部分参数都为0，这就大大压缩了模型权重及特征向量的维度。**Wide部分模型训练完之后留下来的特征都是非常重要的，那么模型的“记忆能力”就可以理解为发现"直接的"，“暴力的”，“显然的”关联规则的能力**。例如Google W&D期望wide部分发现这样的规则：**用户安装了应用A，此时曝光应用B，用户安装应用B的概率大。**
...
W&D模型是将两部分输出的结果结合起来联合训练，将deep和wide部分的输出重新使用一个**逻辑回归模型**做最终的预测，输出概率值。联合训练的数学形式如下：需要注意的是，因为Wide侧的数据是高维稀疏的，所以作者使用了FTRL算法优化，而Deep侧使用的是 Adagrad。
$$
